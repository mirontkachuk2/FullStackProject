 ...
 */
export async function addTask(title: string, position: number) {
    try {
        const token = await getToken();
 ...
 * @returns {Promise<Object>} The response data
 */
export async function reorderTasks(updatedTasks: { id: string; position: number }[]) {
    const token = await getToken();
 ...
 */
export async function getTaskImages(taskTitle: string) {
    try {
